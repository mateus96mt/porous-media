...
c
      ...
      gfe = 0.d00
      if(nel.gt.1.and.nel.lt.40) gfe = gf3/h2
      if(nel.gt.1561.and.nel.lt.1600) gfe = -gf3/h2
c      
...
c
	...
	write(iecho,*) 'lado', ndgs, ' u.n = 0.0'
      call neuhbc(shgpn,shlb,detpn,wn,
...
c
            ...
            write(1,1111)xl(1,1),xl(2,1),dvel(1,1,nel),dvel(2,1,nel)
         if  (mod(nel,int(sqrt(real(numel)))).eq.0)then
            write(1,1111)xl(1,2),xl(2,2),dvel(1,2,nel),dvel(2,2,nel)
        end if


         if(nel.eq.numel) then
            i = numel-int(sqrt(real(numel)))
            k =int(sqrt(real(numel)))
           do j=1,k
            write(1,1111) xl(1,4),xl(2,4),dvel(1,4,i+j),dvel(2,4,i+j)
           end do
         end if

         if(nel.eq.numel) write(1,1111)xl(1,3),xl(2,3),dvel(1,3,nel),dvel(2,3,nel)
